...
import com.dizang.task.ThreadA;
...
public class WebSocket {
    ...
    private Thread thread = new Thread(threadA);
    ...
    public void onMessage(Integer userId) {
        try {
            ...
            for(Session session : sessions) {
                session.getBasicRemote().sendText("这是推送测试数据！您刚发送的消息是：修改了");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
